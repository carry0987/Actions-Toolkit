import { ListOpts } from '@/interfaces/util';
...
export class Util {
    ...
    public static async asyncForEach<T>(array: T[], callback: (item: T, index: number, array: T[]) => Promise<void>) {
    ...
    public static async powershellCommand(script: string, params?: Record<string, string>) {
        const powershellPath: string = await which('powershell', true);
        ...
        return {
            ...
            args: [
                ...
                'Unrestricted',
                '-Command',
                `& '${escapedScript}' ${escapedParams.join(' ')}`
            ]
        };
    }

    public static isDirectory(p: PathLike): boolean {
        try {
    ...
    public static isPathRelativeTo(parentPath: string, childPath: string): boolean {
        const rpp = resolve(parentPath);
        const rcp = resolve(childPath);

        return rcp.startsWith(rpp.endsWith(sep) ? rpp : `${rpp}${sep}`);
    }
